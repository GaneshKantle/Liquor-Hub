...
<body class="relative flex min-h-screen flex-col items-center justify-center overflow-hidden px-4 py-10 font-sans text-ink antialiased"
  style="background-color:#f8f5ef; background-image: radial-gradient(ellipse 70% 50% at 10% 0%, rgba(184,115,51,0.16), transparent 50%), radial-gradient(ellipse 50% 40% at 100% 100%, rgba(243,228,212,0.8), transparent 50%), linear-gradient(165deg, #fff, #f8f5ef);">
  ...
  <div class="glass-strong relative w-full max-w-md rounded-[2rem] p-6 sm:p-8">
    ...
    <form action="login" method="POST" class="space-y-4">
      ...
      <button type="submit" class="inline-flex w-full min-h-11 items-center justify-center rounded-full bg-copper text-sm font-semibold text-white shadow-md shadow-copper/25 transition hover:bg-copper-strong">Sign in</button>
    </form>

    <div class="mt-5 text-center text-sm text-ink-muted">
      <a href="forgetPassword" class="font-medium text-copper-strong hover:underline">Forgot password?</a>
      <span> · </span>
      <a href="register.jsp" class="font-medium text-copper-strong hover:underline">Create account</a>
      <span> · </span>
      <a href="<%=request.getContextPath()%>/home" class="font-medium text-copper-strong hover:underline">Back to shop</a>
    </div>
  </div>
</body>
</html>
