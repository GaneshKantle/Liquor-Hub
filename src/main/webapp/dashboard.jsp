...
<body class="relative min-h-screen overflow-x-clip font-sans text-ink antialiased"
  style="background-color:#f8f5ef; background-image: radial-gradient(ellipse 60% 40% at 0% 0%, rgba(184,115,51,0.12), transparent 50%), linear-gradient(180deg, #fff, #f8f5ef);">
<%
...
%>
  ...
  <div class="pointer-events-none absolute -right-20 top-40 h-72 w-72 rounded-full bg-copper/10 blur-3xl"></div>
  ...
  <header class="sticky top-0 z-20 px-4 pt-3 sm:px-6">
    <div class="glass mx-auto flex max-w-3xl flex-wrap items-center gap-3 rounded-full px-4 py-2.5">
      <a href="<%=request.getContextPath()%>/home" class="mr-auto font-display text-2xl font-semibold tracking-tight">LiquorHub</a>
  ...
  <main class="relative mx-auto max-w-3xl px-4 py-8 sm:px-6 sm:py-10">
    <div class="glass-strong rounded-[2rem] p-5 sm:p-8">
      ...
      <h3 class="mb-4 font-display text-xl font-semibold text-copper-strong">Update details</h3>
      <form action="update" method="POST" class="space-y-4">
        <div>
          ...
          <input id="name" type="text" name="name" value="<%= customer.getName() %>" required class="w-full min-h-11 rounded-2xl border border-white/80 bg-white/50 px-3.5 text-sm outline-none backdrop-blur transition focus:border-copper/40 focus:bg-white/80 focus:ring-2 focus:ring-copper/15">
        </div>
        <div>
          <label for="mail" class="mb-1.5 block text-[0.72rem] font-bold uppercase tracking-wider text-ink-muted">Email</label>
          <input id="mail" type="email" name="mail" value="<%= customer.getEmail() %>" required class="w-full min-h-11 rounded-2xl border border-white/80 bg-white/50 px-3.5 text-sm outline-none backdrop-blur transition focus:border-copper/40 focus:bg-white/80 focus:ring-2 focus:ring-copper/15">
        </div>
        <div>
          <label for="phone" class="mb-1.5 block text-[0.72rem] font-bold uppercase tracking-wider text-ink-muted">Phone</label>
          <input id="phone" type="text" name="phone" value="<%= customer.getPhone() %>" required class="w-full min-h-11 rounded-2xl border border-white/80 bg-white/50 px-3.5 text-sm outline-none backdrop-blur transition focus:border-copper/40 focus:bg-white/80 focus:ring-2 focus:ring-copper/15">
        </div>
        <div>
          <label for="address" class="mb-1.5 block text-[0.72rem] font-bold uppercase tracking-wider text-ink-muted">Address</label>
          <input id="address" type="text" name="address" value="<%= customer.getAddress() %>" required class="w-full min-h-11 rounded-2xl border border-white/80 bg-white/50 px-3.5 text-sm outline-none backdrop-blur transition focus:border-copper/40 focus:bg-white/80 focus:ring-2 focus:ring-copper/15">
        </div>
        <div>
          <label for="password" class="mb-1.5 block text-[0.72rem] font-bold uppercase tracking-wider text-ink-muted">Password</label>
          <input id="password" type="password" name="password" placeholder="Leave blank to keep current" class="w-full min-h-11 rounded-2xl border border-white/80 bg-white/50 px-3.5 text-sm outline-none backdrop-blur transition focus:border-copper/40 focus:bg-white/80 focus:ring-2 focus:ring-copper/15">
          <p class="mt-1 text-xs text-ink-muted">Only fill this if you want to change your password here.</p>
        </div>
        <button type="submit" class="inline-flex min-h-11 items-center justify-center rounded-full bg-copper px-6 text-sm font-semibold text-white shadow-md transition hover:bg-copper-strong">Save changes</button>
      </form>
    </div>
  </main>
